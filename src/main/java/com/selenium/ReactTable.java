...
public class ReactTable  {
	...
	public ReactTable(Object element) {
		if(element instanceof WebElement) {
			this.locator = page.getByObjectFromWebElement(element);
		}else {
	...
	private List<WebElement> getRowsList() {
		if(rowList == null) {
			...
			try{ 
				...
				Reporter.ERROR("getRowsList() method failed for locator "+locator+" <br/><br/> Outer HTML "+getWebElement().getAttribute("outerHTML"));
				new Custom_ExceptionHandler(e);
			}
		}
		return rowList;
	...
	public ReactTable initHeaderColumnList(int headerRow) {
		try{
			...
			Reporter.WARNING( "initHeaderColumnList() method failed for headerRow "+headerRow+" in table "+locator+" <br/><br/> Exception message "+e.getMessage());
			//new CustomExceptionHandler(e);
		}
		return this;
	}
	...
	public void verifyColumnHeaders(int headerRow, List<String> expectedColNames) {
		...
		for (int i = 0; i < expectedColNames.size(); i++) {
			...
			if (!tempColName.toLowerCase().contains(expectedColNames.get(i).toLowerCase())) {
				Reporter.FAIL( "On Row["+headerRow+"] and Col["+(i+1)+"] Column text '" + expectedColNames.get(i) + "' not found but column '"
		...
		if (flag) {
			Reporter.PASS( "All columns found : " + expectedColNames);
		} else {
			Reporter.INFO( "COLUMNS EXPECTED: [" + getStringifiedColNames(expectedColNames) + "]");
			Reporter.INFO( "COLUMNS ACTUAL : [" + getStringifiedColNames(actualColNames) + "]");
		}
	}
