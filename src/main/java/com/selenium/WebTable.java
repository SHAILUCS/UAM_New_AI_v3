...
public class WebTable {
	...
	private WebElement getWebElement() {
		...
		try {
			...
			elem = page.getInteractableWebElementFromList(locator);
		} catch (Exception e) {
	...
	/**
	 * This method provides you the list<WebElement> of all rows of the table, whose
	 * locator was passed in WebTable class constructor
	 * 
	...
	private List<WebElement> getRowsList() {
		...
		try {
			...
			// Getting null pointer exception on this line, even though the table is getting
			// displayed
			...
			Reporter.FAIL("getRowsList() method failed for locator " + locator + " <br/><br/> Outer HTML "
					+ getWebElement().getAttribute("outerHTML"));
			new Custom_ExceptionHandler(e);
		}
		return listTR;
	}
	 ...
	 * 
	 * @return int rowCount the count of tr, and in case of exception will return -1
	 */
	 ...
	 * 
	 * @param rowNum The Row number, from which the column count is need to be
	 *               fetched
	 * @return int colCount the count of td inside tr, and in case of exception will
	 *         return -1
	 */
	...
	/**
	 * use this method to get the String innerText of the cell pointed by rowNum and
	 * colNum
	 * 
	 * @param rowNum The Row number of cell
	 * @param rowNum The Column number of cell
	 * @return The innerText displayed in the cell, and in case of exception will
	 *         return null
	 */
	public String getCellText(int rowNum, int colNum) {
		...
		try {
			...
			page.javaScript_ScrollIntoMIDDLEView_AndHighlight(cellObj);
	...
	/**
	 * use this method to get the WebElement Object(
	 * <td>) pointed by rowNum and colNum. This method is useful when we want to
	 * apply getAttribute method on the Table cell itself.
	 * 
	 * @param rowNum The Row number of cell
	 * @param colNum The Column number of cell
	 * @return The WebElement Object, and in case of exception will return null
	 */
	...
	/**
	 * use this method to get the List of WebElements matching the passed Tag(for
	 * e.g. "img","a" etc) present in the cell(pointed by passed row and col num)
	 * 
	 * @param rowNum The Row number of cell
	 * @param rowNum The Column number of cell
	 * @param tag    The tag of desired webelement, it could be any valid html tag,
	 *               common examples are "a","img".
	 * @return The matching tags inside the passed cell, it will skip all hierarchy
	 *         and fetch the desired objects list, and in case of exception will
	 *         return null
	 */
	...
	/**
	 * use this method to get the single WebElement matching the passed Tag(for e.g.
	 * "img","a" etc) present in the cell(pointed by passed row and col num) based
	 * on passed index
	 * 
	 * @param rowNum The Row number of cell
	 * @param rowNum The Column number of cell
	 * @param tag    The tag of desired webelement, it could be any valid html tag,
	 *               common examples are "a","img".
	 * @param index  The index of desired webelement from the list, index starts
	 *               with 0
	 * @return The element which is present on the passed index, matching the tag in
	 *         a cell pointed by passed row and col nums, and in case of exception
	 *         will return null
	 */
	public WebElement getChildObject(int rowNum, int colNum, String tag, int index) {
		...
		try {
			...
			page.javaScript_ScrollIntoMIDDLEView_AndHighlight(getCellObject(rowNum, colNum));
		} catch (Exception e) {
			// new CustomExceptionHandler(e);
		}
		return childObject;
	...
	/**
	 * use this method to get the count of WebElements matching the passed Tag(for
	 * e.g. "img","a" etc) present in the cell(pointed by passed row and col num)
	 * 
	 * @param rowNum The Row number of cell
	 * @param rowNum The Column number of cell
	 * @param tag    The tag of desired webelement, it could be any valid html tag,
	 *               common examples are "a","img".
	 * @return The count of webelement, matching the tag in a cell pointed by passed
	 *         row and col nums, and in case of exception will return -1
	 */
	 ...
	 * 
	 * @param visibleTxt The visible text, for which the row count is needed to be
	 *                   found
	 ...
	 * use this method to get the Row number which has the passed text value
	 * 
	 * @param startRow   the begin row count, inclusive.
	 * @param visibleTxt The visible text, for which the row count is needed to be
	 *                   found
	 * @return The first row number that has the text which matches the passed
	 *         parameter value, and in case of exception will return -1
	...
	public WebTable initHeaderColumnList(int headerRow) {
		try {
			...
			for (WebElement temp : thList) {
				page.javaScript_ScrollIntoMIDDLEView_AndHighlight(temp);
				headerColumnList.add(temp.getText().replace("\r\n", " ").replace("\n", " "));
			}
		} catch (Exception e) {
			Reporter.WARNING("initHeaderColumnList() method failed for headerRow " + headerRow + " in table " + locator
	...
	public int getColumnNumber(String colName) {
		...
		try {
			if (headerColumnList != null) {
				for (int colNum = 0; colNum < headerColumnList.size(); colNum++) {
					if (headerColumnList.get(colNum).contains(colName)) {
		...
		if (foundColNum == -1) {
			Reporter.INFO("Passed Column [" + colName + "] is not found in column header list " + headerColumnList);
		}
