...
public class MailManager {
	...
	private final static String REPORT_TEMPLATE_FILE_NAME = FrameworkConfig.REPORT_TEMPLATE_NAME;
	...
	public static void main(String[] args) {
		 ...
		 * 
		 * int startIndex = body.indexOf("https://"); int endIndex = body.indexOf("\"",
		 * startIndex); System.out.println(startIndex); System.out.println(endIndex);
		 * 
		 * String url = body.substring(startIndex, endIndex); System.out.println(url);
		 */
	...
	public static void sendNotificationMail(String desc) {
		...
		System.out.println("===============================================================================");
		System.out.println("Mail feature STARTED " + new Date());
		if (!FrameworkConfig.ENABLE_EMAIL_NOTIFICATIONS) {
			System.out.println("Mail feature is STOPPED " + new Date());
			System.out.println("===============================================================================");
			return;
		}
		// ExcelManager data = new ExcelManager(Config.getTestDataFilePath(), EMAIL_CRED_SHEET);
		...
		try {
			...
			try {
				...
				try {
					...
					String filename = FrameworkConfig.getResultHtmlFilePath();
					DataSource source = new FileDataSource(filename);
					messageBodyPart.setDataHandler(new DataHandler(source));
					messageBodyPart.setFileName(REPORT_TEMPLATE_FILE_NAME);
					multipart.addBodyPart(messageBodyPart);
				} catch (Exception ee) {
					ee.printStackTrace();
				}
				...
				try {
					...
					String filename1 = FrameworkConfig.getResultextenthtmlfilePath();
					DataSource source1 = new FileDataSource(filename1);
					messageBodyPart1.setDataHandler(new DataHandler(source1));
					messageBodyPart1.setFileName(FrameworkConfig.HTML_REPORT_FILE_NAME_EXTENT);
				 ...
				 */
				/*
				 * try { BodyPart messageBodyPart1 = new MimeBodyPart(); Path file =
				 * Paths.get("C:/",
				 * "Users/GWL/git/AmpAutomation/src/test/java/com/mail/Report.zip");
				 * messageBodyPart1.setDataHandler(new DataHandler(new
				 * FileDataSource(file.toFile())));
				 * messageBodyPart1.setFileName(file.getFileName().toString());
				 * multipart.addBodyPart(messageBodyPart1); System.out.println("attached ZIP");
				...
				messageWoAttchment.setRecipients(Message.RecipientType.TO,
						InternetAddress.parse("")); //TODO data.getValue(getAppropriateRow(desc), EMAIL_TO_COL)
				messageWoAttchment
			...
			if (e.toString().contains("AuthenticationFailed")) {
				System.err.println(
						"FAILED TO SEND MAIL : Authentication Failed for the username and password mentioned in Sheet ["
								+ EMAIL_CRED_SHEET + "] ") ; // + Config.getTestDataFilePath())
				e.printStackTrace();
			} else {
				e.printStackTrace();
			}
	...
	private static Object getMailBody(String desc) {
		return "Hi All," + "<br/><br/>" + "This is the System generated mail for Automation Testing." + "<h3>" + desc
				...
				+ ReportingHistoryHTML.getDashboardContent(FrameworkConfig.getResultHtmlFilePath()) + "<br/> "
				+ "<h4>Scenarios: </h4>"
				+ ReportingHistoryHTML.getQuickViewContent(FrameworkConfig.getResultHtmlFilePath())
	...
	public static Map<String, String> readMail_SpamFolder(String... mailBodyContains) {
		return readMail_SpamFolder(FrameworkConfig.WAIT_EXPLICIT, mailBodyContains);
	}
	...
	public static Map<String, String> readMail_SpamFolder(long timeOutInSec, String... mailBodyContains) {
		...
		try {
			...
			do {
				...
				if (timeOutTimeCounter > 0) {
					...
					timeOut = false;
					Reporter.FAIL( "TIMEOUT MAIL [" + mailBodyContains + "] NOT FOUND AFTER WAITING FOR '"
		...
		if (foundMessage != null) {
			Reporter.PASS( "Mail " + foundMessage + " is received");
		} else {
			Reporter.FAIL( "Mail " + mailBodyContains + " is NOT received");
		}
